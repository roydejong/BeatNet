...
// Modifications were made by SoftwareGuy (Coburn). Copyright (c) 2019 Matt Coburn (SoftwareGuy/Coburn64), Chris Burns (c6burns)
...
public class RingBuffer<T>
{
    ...
    public T Dequeue()
    {
        ...
        while
            (_producerCursor.ReadAcquireFence() <
             next) // makes sure we read the data from _entries after we have read the producer cursor
        {
            Thread.SpinWait(1);
        }
        ...
        _consumerCursor
            .WriteReleaseFence(next); // makes sure we read the data from _entries before we update the consumer cursor
        return result;
    }
    ...
    public void Enqueue(T item)
    {
        ...
        this[next] = item;
        _producerCursor
            .WriteReleaseFence(next); // makes sure we write the data in _entries before we update the producer cursor
    }
